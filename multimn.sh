...
# Author: SkyraOfficial
# Source: https://github.com/SkyraGlobal/SKYRA-MultiMN

# TODO:
# - multimn uninstall <prof_name> <number...> => $(echo {3..8}) || seq -s ' ' 3 8
# - multimn reinstall => allow main node ??
# - multimn install <prof_name> -c NUMBER | --count=NUMBER ?? => privkey array, print "MN + dupe_offset : privkey[i]"
# - check and test memory reduction .conf parameters
# - check multimn list [prof] JSON API bugs on unexpected MN values
...
function load_profile() {
	...
	for var in "${CMD_ARRAY[@]}"; do
		if [[ ! "${!prof[@]}" =~ "$var" || -z "${prof[$var]}" ]]; then
			echo -e "Seems like you modified something that was supposed to remain unmodified: ${MAGENTA}$var${NC} parameter should exists and have a assigned value in ${GREEN}.multimn/$1${NC} file"
	...
	if [[ ! "${!conf[@]}" =~ "$1" || -z "${conf[$1]}" || ! $(is_number "${conf[$1]}") ]]; then
		echo -e "Seems like you modified something that was supposed to remain unmodified: ${MAGENTA}$1${NC} parameter should exists and have a assigned number in ${GREEN}.multimn/multimn.conf${NC} file"
		echo -e "You can fix it by adding ${MAGENTA}$1=0${NC} to the .multimn/multimn.conf file (replace the number 0 for the number of nodes installed with multimn using the ${BLUE}$1${NC} profile)"
		echo_json "{\"error\":\"multimn.conf modified\",\"errcode\":102}"
		exit
	fi
...
function find_port() {
	...
	local multi_ports="$(conf_get_value $COIN_FOLDER/$COIN_CONFIG port)"
	[[ ! $(is_number $multi_ports) ]] && multi_ports=$(conf_get_value $COIN_FOLDER/$COIN_CONFIG "masternodeaddr" | rev | cut -d : -f1 | rev)
	[[ ! $(is_number $multi_ports) ]] && multi_ports=$(conf_get_value $COIN_FOLDER/$COIN_CONFIG "externalip"     | rev | cut -d : -f1 | rev)
	for (( i=1; i<=$MULTI_COUNT; i++ )); do
		multi_ports="$multi_ports $(conf_get_value $COIN_FOLDER$i/$COIN_CONFIG rpcport) "
	done
	local port=$(port_check_loop $1 49151 "( $multi_ports )")
	[[ $port ]] && echo $port || echo $(port_check_loop 1024 $1 "( $multi_ports )")
}
function is_number() {
	# <$1 = number>
	[[ "$1" =~ ^[0-9]+$ ]] && echo "1"
...
function configure_systemd() {
	...
	if [[ $1 && ! "$(ps axo cmd:100 | egrep $service_name)" ]]; then
		echo -e "\n${RED}IMPORTANT!!!${NC} \
			...
			\nThe most common causes of this might be that either you made something to a file that multimn modifies or creates, or that you don't have enough free resources (usually memory).
...
function wallet_cmd() {
	# <$1 = start|stop|loaded> | [$2 = multi_number] | [$3 = wait_timeout(loaded)]
	exec 2> /dev/null
...
function cmd_profadd() {
	...
	echo -e "${BLUE}$prof_name${NC} profile successfully added, use ${GREEN}multimn install $prof_name${NC} to create a new instance of the masternode"

	local retcode=0
	if [[ -z "${prof[COIN_SERVICE]}" ]]; then
		echo -e "\n${YELLOW}WARNING:${NC} The provided profile doesn't have a ${CYAN}\"COIN_SERVICE\"${NC} parameter, the multimn script won't be able to stop the main node on some commands"
		if [[ -t 1 ]]; then
		...
		if [[ -f /etc/systemd/system/${prof[COIN_NAME]}.service ]]; then
			...
			conf_set_value ~/.multimn/$prof_name "COIN_SERVICE" "\"${prof[COIN_NAME]}.service\"" 1
			retcode=1
...
function cmd_profdel() {
	...
	local deleted_multies=$MULTI_COUNT
	if [[ $MULTI_COUNT -gt 0 ]]; then
		if [[ -t 1 ]]; then
			read -r -p "All the multies created with this profile will be deleted, are you sure to apply this command? [Y/n]`echo $'\n> '`" yesno
	...
	sed -i "/$PROFILE_NAME\=/d" .multimn/multimn.conf
	sed -i "/^$/d"   .multimn/multimn.conf

	rm -rf /usr/bin/$COIN_DAEMON-0
	rm -rf /usr/bin/$COIN_DAEMON-all
	rm -rf /usr/bin/$COIN_CLI-0
	rm -rf /usr/bin/$COIN_CLI-all
	rm -rf .multimn/$PROFILE_NAME

	echo_json "{\"message\":\"profile successfully deleted\",\"deleted\":true,\"count\":$deleted_multies}"
}
function cmd_install() {
	...
	[[ ! $(grep "addnode=152.42.208.29" $new_folder/$COIN_CONFIG) ]] && echo "addnode=152.42.208.29" >> $new_folder/$COIN_CONFIG
	...
	$(conf_set_value .multimn/multimn.conf $PROFILE_NAME $1 1)

	if [[ ! $NEW_KEY ]]; then
		# main and multies were stopped on createmasternodekey
	...
	if [[ $INSTALL_BOOTSTRAP ]]; then
		...
		elif [[ $MULTI_COUNT -ge 2 && $1 -eq 1 ]]; then # only for reinstall
			echo -e "${MAGENTA}NOTE:${NC} Can't stop the main node, trying the bootstrap with the dupe ${CYAN}2${NC}"
			cmd_bootstrap 2 $1 3>/dev/null
		else
			echo -e "${YELLOW}WARNING:${NC} Can't automatically apply the bootstrap, use ${GREEN}multimn bootstrap${NC} manually"
			((retcode+=1))
		fi
	fi
	...
	echo -e "===================================================================================================\
			\n${BLUE}$COIN_NAME${NC} multiple masternode ${CYAN}number $1${NC} should be now up and trying to sync with the blockchain.\
			\nThe multiple masternode uses the $([[ $show_ip ]] && echo "IP:PORT ${YELLOW}$show_ip:$([[ $mn_port ]] && echo $mn_port || echo ????)${NC}" || echo "same IP and PORT than the original one").\
			\nRPC port is ${MAGENTA}$NEW_RPC${NC}, this one is used to send commands to the wallet, DON'T put it in 'masternode.conf' (other programs might want to use this port which causes a conflict, but you can change it with ${MAGENTA}multimn rpcchange $PROFILE_NAME $1 PORT_NUMBER${NC}).\
			...
			\nMULTIPLE MASTERNODE PRIVATEKEY is: ${GREEN}$NEW_KEY${NC}\
			...
			\nNOTE 1: ${GREEN}$COIN_CLI-0${NC} and ${GREEN}$COIN_DAEMON-0${NC} are just a reference to the 'main masternode', not a created one with multimn.\
...
function cmd_reinstall() {
	...
	$(conf_set_value .multimn/multimn.conf $PROFILE_NAME $MULTI_COUNT)
}
function cmd_uninstall() {
	# <$1 = instance_number/all>

	if [ $1 == "all" ]; then
		for (( i=$MULTI_COUNT; i>=1; i-- )); do
		...
		$(conf_set_value .multimn/multimn.conf $PROFILE_NAME 0 1)
		$(make_chmod_file /usr/bin/$COIN_CLI-all    "#!/bin/bash\nfor (( i=0; i<=0; i++ )) do\n echo -e MN\$i:\n $COIN_CLI-\$i \$@\ndone")
		$(make_chmod_file /usr/bin/$COIN_DAEMON-all "#!/bin/bash\nfor (( i=0; i<=0; i++ )) do\n echo -e MN\$i:\n $COIN_DAEMON-\$i \$@\ndone")
		systemctl daemon-reload
		echo_json "{\"message\":\"dupe/s successfully uninstalled\",\"count\":$MULTI_COUNT,\"multies\":[$(seq -s ',' 1 $MULTI_COUNT)]}"
	else
		echo -e "Uninstalling ${BLUE}$PROFILE_NAME${NC} instance ${CYAN}number $1${NC}"
		wallet_cmd stop $1 > /dev/null
		rm -rf /usr/bin/$COIN_CLI-$MULTI_COUNT
		rm -rf /usr/bin/$COIN_DAEMON-$MULTI_COUNT
		rm -rf $COIN_FOLDER$1
		$(make_chmod_file /usr/bin/$COIN_CLI-all    "#!/bin/bash\nfor (( i=0; i<=$(($MULTI_COUNT-1)); i++ )) do\n echo -e MN\$i:\n $COIN_CLI-\$i \$@\ndone")
		$(make_chmod_file /usr/bin/$COIN_DAEMON-all "#!/bin/bash\nfor (( i=0; i<=$(($MULTI_COUNT-1)); i++ )) do\n echo -e MN\$i:\n $COIN_DAEMON-\$i \$@\ndone")
		$(conf_set_value .multimn/multimn.conf $PROFILE_NAME $(($MULTI_COUNT-1)) 1)
		...
		systemctl disable $COIN_NAME-$MULTI_COUNT.service &> /dev/null
		rm -rf /etc/systemd/system/$COIN_NAME-$MULTI_COUNT.service
		systemctl daemon-reload
		echo_json "{\"message\":\"dupe/s successfully uninstalled\",\"count\":1,\"multies\":[$1]}"
	fi
}
function cmd_bootstrap() {
	...
	if [[ $1 -eq $2 ]]; then
		...
		if [[ ! $COIN_SERVICE || ! -f /etc/systemd/system/$COIN_SERVICE ]]; then
			...
			[[ $2 -eq 0 ]] && echo -e "Optionally you can put use a dupe as source of the chain files, example: ${YELLOW}multimn bootstrap PROFILE 2 1${NC} (copy dupe 1 to dupe 2)."
	...
	#For copied bootstrap data while new node installation // Ex. : multimn install skyra --bootstrap
    cp -r $(get_folder $1)blocks $(get_folder $2)
    cp -r $(get_folder $1)chainstate $(get_folder $2)
    cp -r $(get_folder $1)sporks $(get_folder $2)
    cp -r $(get_folder $1)evodb $(get_folder $2)	
...
function cmd_ipmod() {
	...
	if [[ ! $netmask ]]; then
		...
		echo -e "${RED}ERROR:${NC} Interface ${GREEN}$4${NC} doesn't exists, use ${YELLOW}multimn iplist${NC} to see the existing interfaces"
		echo_json "{\"error\":\"Interface doesn't exists\",\"errcode\":702}"
		return
	elif [[ $(echo "$iface" | wc -l) -gt 1 ]]; then
		echo -e "${RED}ERROR:${NC} There are 2 or more available interfaces, you'll have to specify it as an extra parameter, use ${YELLOW}multimn iplist${NC} to see the existing interfaces"
	...
	if [[ ! $ip_res ]]; then
		touch /etc/init.d/multimn_ipmanage
		local initd=$(cat /etc/init.d/multimn_ipmanage | grep "^ip")
		if [[ $1 == "add" ]]; then
		...
		echo -e "#!/bin/sh -e\
		\n### BEGIN INIT INFO\
		\n# Provides:          multimn_ipmanage\
		\n# Required-Start:    \$remote_fs \$syslog\
		\n# Required-Stop:     \$remote_fs \$syslog\
		\n# Default-Start:     5\
		\n# Default-Stop:\
		\n# Short-Description: Start ips at boot time\
		\n# Description:       multimn script ip manager to keep ips enabled after reboot\
		\n### END INIT INFO\
		\n\
		\n$initd\
		\n\
		\nexit 0" > /etc/init.d/multimn_ipmanage
		chmod +x /etc/init.d/multimn_ipmanage
		update-rc.d multimn_ipmanage defaults
...
function cmd_systemctlall() {
	...
	if [[ $COIN_SERVICE ]]; then
		...
		echo -e "${MAGENTA}Main MN service not detected in the profile, applying command to multies only${NC}"
	fi
	for (( i=1; i<=$MULTI_COUNT; i++ )); do
...
function cmd_list() {
	...
	function print_multi_info() {
		...
		js_multies+=("{$(array_join , "${js_params[@]}")}")
	}
	...
	if [[ ! $1 ]]; then
		...
		echo -e "Total count : $(echo ${conf[@]} | tr ' ' '\n' | cut -d '=' -f2 | awk '{ SUM += $1 } END { print SUM }') multies (+ $(echo ${#conf[@]}) main nodes)"
		...
		for prof in $profs; do
			...
			if [[ $check_prof ]]; then
				...
				load_profile $prof 1
				local js_multies=()
				echo -e "${BLUE}$prof${NC}: ${CYAN}$MULTI_COUNT${NC} created nodes with multimn"
				...
				js_profs+=("{\"name\":\"$prof\",\"count\":$MULTI_COUNT,\"mn\":[$(array_join , "${js_multies[@]}")]}")
			fi
...
function cmd_swapfile() {
	...
	[[ -f /mnt/multimn_swapfile ]] && swapoff /mnt/multimn_swapfile &> /dev/null

	if [[ $1 -eq 0 ]]; then
		rm -rf /mnt/multimn_swapfile
		sed -i "/\/mnt\/multimn_swapfile/d" /etc/fstab
		...
		dd if=/dev/zero of=/mnt/multimn_swapfile bs=1024 bs=1M count=$1 status=progress
		chmod 600 /mnt/multimn_swapfile &> /dev/null
		mkswap /mnt/multimn_swapfile &> /dev/null
		swapon /mnt/multimn_swapfile &> /dev/null
		/mnt/multimn_swapfile swap swap defaults 0 0 &> /dev/null
		[[ ! $(cat /etc/fstab | grep "/mnt/multimn_swapfile") ]] && echo "/mnt/multimn_swapfile none swap 0 0" >> /etc/fstab
...
function cmd_help() {
	echo -e "Options:\
			\n  - ${YELLOW}multimn profadd <prof_file> [prof_name]          ${NC}Adds a profile that will be used to create multiples of the masternode, it will use the COIN_NAME parameter as name if a prof_name is not provided.\
			\n  - ${YELLOW}multimn profdel <prof_name>                      ${NC}Deletes the given profile name, this will uninstall too any dupe that uses this profile.\
			\n  - ${YELLOW}multimn install <prof_name> [params...]          ${NC}Install a new dupe based on the parameters of the given profile name.\
			...
			\n  - ${YELLOW}multimn reinstall <prof_name> <node> [params...] ${NC}Reinstalls the specified dupe, this is just in case if the dupe is giving problems.\
			\n      ${YELLOW}[params...]${NC} list:\
			\n        ${GREEN}-i ${DARKCYAN}IP${NC},  ${GREEN}--ip=${DARKCYAN}IP${NC}       Use a specific IPv4 or IPv6.\
			\n        ${GREEN}-r ${DARKCYAN}RPC${NC}, ${GREEN}--rpcport=${DARKCYAN}RPC${NC} Use a specific port for RPC commands (must be valid and not in use).\
			\n        ${GREEN}-p ${DARKCYAN}KEY${NC}, ${GREEN}--privkey=${DARKCYAN}KEY${NC} Set a user-defined masternode private key.\
			\n        ${GREEN}-b${NC},     ${GREEN}--bootstrap${NC}   Apply a bootstrap during the reinstallation.\
			\n  - ${YELLOW}multimn uninstall <prof_name> <node|all>         ${NC}Uninstall the specified node of the given profile name, you can put ${YELLOW}all${NC} instead of a node number to uninstall all the multiple instances.\
			\n  - ${YELLOW}multimn bootstrap <prof_name> <node_1> <node_2>  ${NC}Copies the chain from node_1 to node_2.\
			\n  - ${YELLOW}multimn iplist                                   ${NC}Shows all your configurated IPv4 and IPv6.\
			\n  - ${YELLOW}multimn ipadd <ip> <netmask> [interface]         ${NC}Allows the system to recognize a new IPv4 or IPv6.\
			\n  - ${YELLOW}multimn ipdel <ip> <netmask> [interface]         ${NC}Deletes an already recognized IPv4 or IPv6.\
			\n  - ${YELLOW}multimn rpcchange <prof_name> <node> [port]      ${NC}Changes the RPC port used from the given node with the new one (or finds a new one by itself if no port is given).\
			\n  - ${YELLOW}multimn systemctlall <prof_name> <command>       ${NC}Applies the systemctl command to all the multiple instances of the given profile name.\
			\n  - ${YELLOW}multimn list [prof_names...] [params...]         ${NC}Shows the amount of multiple instances of every masternode, if a profile name/s are provided, it lists an extended info of the profile/s instances.\
			\n      ${YELLOW}[params...]${NC} list:\
			...
			\n  - ${YELLOW}multimn checkmem                                 ${NC}Shows the memory usage (%) of every group of nodes.\
			\n  - ${YELLOW}multimn swapfile <size_in_mbytes>                ${NC}Creates, changes or deletes (if parameter is ${CYAN}0${NC}) a swapfile of the given size in MB to increase the virtual memory.\
			\n  - ${YELLOW}multimn update                                   ${NC}Checks the last version of the script and updates it if necessary.\
			...
			\n**NOTE 2**: Check ${CYAN}https://github.com/SkyraGlobal/SKYRA-MultiMN/wiki/Commands${NC} for extended info and usage examples of each command.\
			\n**NOTE 3**: Check ${CYAN}https://github.com/SkyraGlobal/SKYRA-MultiMN/wiki/FAQs${NC} for technical questions and troubleshooting."
}
function cmd_update() {
	curl -sL https://raw.githubusercontent.com/SkyraGlobal/SKYRA-MultiMN/main/multimn_install.sh 3>&3 | bash
	exit
}
...
function main() {
	...
	function instance_valid() {
		...
		if [[ ! $(is_number $1) ]]; then
			...
			exit
		elif [[ $1 -gt $MULTI_COUNT ]]; then
			echo -e "Instance ${CYAN}$(stoi $1)${NC} doesn't exists, there are only ${CYAN}$MULTI_COUNT${NC} instances of ${BLUE}$PROFILE_NAME${NC}"
	...
	case "$1" in
		"profadd")
			exit_no_param "$2" "${YELLOW}multimn profadd <prof_file> [prof_name]${NC} requires a profile file and optionally a new profile name as parameters"
			cd $curr_dir
			cmd_profadd $2 $3
			;;
		"profdel")
			exit_no_param "$2" "${YELLOW}multimn profadd <prof_name>${NC} requires a profile name as parameter"
			load_profile $2
			cmd_profdel
			;;
		"install")
			exit_no_param "$2" "${YELLOW}multimn install <prof_name> [params...]${NC} requires a profile name of an added profile as a parameter"
			load_profile $2 1
			opt_install_params "${@:3}"
			cmd_install $(($MULTI_COUNT+1))
			;;
		"reinstall")
			exit_no_param "$3" "${YELLOW}multimn reinstall <prof_name> <node> [params...]${NC} requires a profile name and a node number as parameters"
			load_profile $2 1
			instance_valid $3
			...
			;;
		"uninstall")
			exit_no_param "$3" "${YELLOW}multimn uninstall <prof_name> <node|all>${NC} requires a profile name and a node number (or all) as parameters"
			load_profile $2
			[[ $3 != "all" ]] && instance_valid $3
			cmd_uninstall $([[ $3 == "all" ]] && echo "all" || echo $(stoi $3))
			;;
		"bootstrap")
			exit_no_param "$4" "${YELLOW}multimn bootstrap <prof_name> <node_1> <node_2>${NC} requires a profile name and 2 node numbers as parameters"
			load_profile $2 1
			instance_valid $3 1
			instance_valid $4 1
		...
		"iplist")
			...
			;;
		"ipadd")
			exit_no_param "$3" "${YELLOW}multimn ipadd <ip> <netmask> [interface]${NC} requires a IP, a netmask and a interface name (if there's more than 1)"
			ip_parse $2
			cmd_ipmod "add" $2 $3 $4
			;;
		"ipdel")
			exit_no_param "$3" "${YELLOW}multimn ipdel <ip> <netmask> [interface]${NC} requires a IP, a netmask and a interface name (if there's more than 1)"
			ip_parse $2
			cmd_ipmod "del" $2 $3 $4
			;;
		"rpcchange")
			exit_no_param "$3" "${YELLOW}multimn rpcchange <prof_name> <node> [port]${NC} requires a profile name, node number and optionally a port number as parameters"
			load_profile $2 1
			instance_valid $3 1
			cmd_rpcchange $(stoi $3) $4
			;;
		"systemctlall")
			exit_no_param "$3" "${YELLOW}multimn systemctlall <prof_name> <command>${NC} requires a profile name and a command as parameters"
			load_profile $2
			cmd_systemctlall $3
			;;
		"list")
			...
			;;
		"swapfile")
			exit_no_param "$2" "${YELLOW}multimn swapfile <size_in_mbytes>${NC} requires a number as parameter"
			cmd_swapfile $(stoi $2)
			;;
		"checkmem")
			cmd_checkmem
